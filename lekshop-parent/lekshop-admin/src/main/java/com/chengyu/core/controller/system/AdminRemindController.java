...
public class AdminRemindController extends AdminBaseController {
    ...
    public CommonResult<CommonPage<UmsMemberRemind>> getNews(Integer page, Integer pageSize) throws Exception{
		List<UmsMemberRemind> list = memberRemindService.getRemindList(getSessionAdmin().getId(), 2, page, pageSize);
    ...
    public CommonResult<Map<String,Object>> countNews() throws Exception{
		Long num = memberRemindService.countUnReadNum(getSessionAdmin().getId(), 2);
    ...
    public CommonResult<String> readAll() throws Exception{
		memberRemindService.readAll(getSessionAdmin().getId(), 2);
