  ...
  <div class="app-container">
    ...
    >
			...
			<el-table-column label="公告类型">
			  <template slot-scope="scope">
			   <span>{{ scope.row.cate == 'user' ? '用户端公告' : '商家端公告' }}</span>
			  </template>
			</el-table-column>
    ...
    <pagination v-show="total>0" :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.pageSize" @pagination="getList" />
		...
		<el-dialog title="编辑" :visible.sync="dialogFormVisible">
		    <el-form ref="dataForm" :rules="rules" :model="notice" label-width="80px" label-position="right" style="width: 100%;">
		      <el-form-item label="标题" prop="cate">
		        <el-tag>{{notice.cate == 'user' ? '用户端公告' : '商家端公告' }}</el-tag>
		      </el-form-item>
					<el-form-item label="是否显示" prop="status">
		        <el-select v-model="notice.status" class="filter-item" placeholder="请选择">
		          <el-option v-for="item in statusOptions" :key="item.key" :label="item.text" :value="item.key" />
		        </el-select>
		      </el-form-item>
					<el-form-item v-if="dialogFormVisible"  prop="content" style="margin-bottom: 30px; width:100%;">
					  <Tinymce ref="editor" v-model="notice.content" :height="300" />
					</el-form-item>
		    </el-form>
...
import {getNoticeList, getNotice, updateNotice} from '@/api/notice'
  ...
  data() {
    return {
      ...
      dialogType: '',
			multipleSelection: [],
			rules: {
			  status: [{ required: true, message: '请选择公告是否显示', trigger: 'blur' }],
      ...
      getNoticeList(this.listQuery).then(response => {
		...
		confirm() {
			this.$refs['dataForm'].validate((valid) => {
			  if (valid) {
			    let formData =Object.assign({}, this.notice)
