<template>
  ...
  <!-- <el-select v-model="listQuery.shopId" placeholder="所属店铺" filterable clearable class="filter-item">
...
<script>
import {getPlatformReportDayList, getPlatformReportMonthList} from '@/api/report'
...
export default {
  ...
  methods: {
    ...
    getDayList() {
      ...
      getPlatformReportDayList(this.listQuery).then(response => {
    ...
    getMonthList() {
      this.listLoading = true
      getPlatformReportMonthList(this.listQuery).then(response => {
        this.list = response.data.list
        this.total = response.data.total
    		this.listLoading = false
      })
