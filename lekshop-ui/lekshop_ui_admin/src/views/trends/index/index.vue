<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="创作号" width="90px"  align="center">
        <template slot-scope="scope">
      		<span>{{ scope.row.walkTrends.walkMemberUid}}</span>
        </template>
      </el-table-column>
      <el-table-column label="创作人" width="120px"  align="center">
        <template slot-scope="scope">
      		<span>{{ scope.row.walkTrends.walkMemberName}}</span>
        </template>
      </el-table-column>
        ...
        <template slot-scope="scope">
          <el-button-group>
           <el-tooltip class="item" effect="dark" content="审核" placement="top">
            <el-button v-if="scope.row.walkTrends.status == 0" type="primary" icon="el-icon-circle-check" size="mini" @click="handleVerify(scope.row.walkTrends)"></el-button>
           </el-tooltip>
    ...
    <el-dialog title="评论" :visible.sync="dialogCommentVisible" >
        ...
        <div v-for="item in commentList" class="comment comment-item">
            <div class="flex">
              ...
              <span style="color:red;font-size: 13px;"class="link-type" @click="handleDeleteComment(item.walkTrendsComment.id)"><i class="el-icon-delete"></i>&nbsp;删除</span>
            </div>
          ...
          <div v-for="item2 in item.chilidCommentList" class="sub-item">
            <div class="flex">
              ...
              <span style="color:red;font-size: 13px;"class="link-type" @click="handleDeleteComment(item2.id)"><i class="el-icon-delete"></i>&nbsp;删除</span>
            </div>
          </div>
...
<script>
import {getTrendsList, publishTrends, deleteTrends, getCommentList, verifyTrends, deleteComment } from '@/api/trends'
    ...
    confirmVerify() {
    	this.$refs['verifyForm'].validate((valid) => {
    	  if (valid) {
    	    ...
    	    verifyTrends(formData).then((response) => {
    				this.getList()
