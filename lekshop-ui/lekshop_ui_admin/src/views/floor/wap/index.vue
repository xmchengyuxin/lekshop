...
import {getFloorDataList, updateFloorData} from '@/api/floor'
export default {
  components: {
    Draggable,
    decorate,
  ...
  methods: {
    ...
    init() {
      ...
      getFloorDataList({templateId: this.$route.query.id}).then((res) => {
        this.contentData = JSON.parse(res.data.oriData);
