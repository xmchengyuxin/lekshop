...
<script>
import {getPointLogList, getAccountTypeSelector} from '@/api/fund'
...
export default {
  ...
  methods: {
		getAccountTypeSelector(){
		  getAccountTypeSelector({type: 2}).then(response => {
    ...
    getList() {
      ...
      getPointLogList(this.listQuery).then(response => {
    ...
    async getDownloadList(query) {
       let list;
       await getPointLogList(query).then(response => {
