      ...
      <el-table-column label="ID" prop="id" align="center" width="65">
        <template slot-scope="scope">
          <span>{{ scope.row.id }}</span>
			...
			<el-table-column label="优惠券类型" width="200px"  align="center">
			  <template slot-scope="scope">
          ...
          <span v-if="scope.row.validityType == 1 ">长期有效</span>
			    <span v-if="scope.row.validityType == 2">领取后当天生效,有效期{{ scope.row.validityDay}}天</span>
            ...
            <p class="tips">温馨提示：95折直接输入9.5</p>
              ...
              <el-radio :label="1" border>长期有效</el-radio>
					...
					<el-form-item v-if="couponConfig.validityType == '2' " label="有效期(天)" prop="validityDay">
					  <el-input v-model.number="couponConfig.validityDay" placeholder="领取后当天生效" style="width: 60%;" />
					</el-form-item>
...
<script>
import {getShopCouponList, updateCoupon, deleteShopCoupon, getShopCateList} from '@/api/shop'
      ...
      dialogType: '',
			...
			rules: {
			  ...
			  type: [{ required: true, message: '请选择优惠券类型', trigger: 'change' }],
				...
				validityDay: [{ required: true, message: '有效期不能为空', trigger: 'blur' }],
        ...
        goodsId: [{ required: true, message: '请选择指定商品', trigger: 'change' }],
  ...
  methods: {
    ...
    getList() {
      ...
      if(this.couponConfig.useGoodsIds){
        formData.goodsId = this.couponConfig.useGoodsIds.split(",")
      }
      if(this.couponConfig.useGoodsCateIds){
        formData.ptid = this.couponConfig.useGoodsCateIds.split(",")
      }
