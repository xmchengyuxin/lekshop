...
<script>
import {getAccountLog, getAccountTypeSelector} from '@/api/fund'
...
export default {
  ...
  methods: {
    getAccountTypeSelector(){
      getAccountTypeSelector({type: 1}).then(response => {
    ...
    getList() {
      ...
      getAccountLog(this.listQuery).then(response => {
    ...
    async getDownloadList(query) {
       let list;
       await getAccountLog(query).then(response => {
