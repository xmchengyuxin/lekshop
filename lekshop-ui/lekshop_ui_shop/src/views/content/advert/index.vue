<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="广告位置" width="150px"  align="center">
        <template slot-scope="scope">
          <el-tag>{{ scope.row.cate | typeFilter}}</el-tag>
        </template>
      </el-table-column>
			<el-table-column label="广告图片" width="300px"  align="center">
			  <template slot-scope="scope">
          <el-image
              fit="scale-down"
              style="height: 100px"
    ...
    <pagination v-show="total>0" :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.pageSize" @pagination="getList" />
		...
		<el-dialog :title="dialogType==='edit'?'编辑':'新增'" :visible.sync="dialogFormVisible">
		    <el-form ref="dataForm" :rules="rules" :model="advert" label-width="80px" label-position="right" style="width: 400px; margin-left:50px;">
          ...
          <el-form-item label="广告位置" prop="cate">
            <el-select v-model="advert.cate" class="filter-item" placeholder="请选择">
              <el-option v-for="item in typeOptions" :key="item.key" :label="item.text" :value="item.key" />
            </el-select>
          </el-form-item>
         ...
         <!-- <el-form-item v-if="advert.url == 'shop' " prop="shopId" label="选择门店">
            <el-select style="width: 150%;" filterable v-model="advert.shopId" placeholder="请选择">
              <el-option v-for="item in shopOptions" :key="item.id" :label="item.name" :value="item.id">
              </el-option>
            </el-select>
          </el-form-item>
					<el-form-item v-if="advert.url == 'buyFood' " label="选择商品" prop="foodId">
					  <el-select
					      style="width:300px;"
					      v-model="advert.foodId"
					      filterable
					      placeholder="请选择商品">
					      <el-option
					...
					<el-form-item label="图片" prop="img">
					  <Upload v-model="advert.img" />
             <ImageSource :mul="false" v-model="advert.img" />
...
<script>
import {getAdvertList, updateAdvert, updateAdvertStatus,deleteAdvert} from '@/api/advert'
...
export default {
  ...
  data() {
    return {
      ...
      dialogType: '',
        ...
        cate: [{ required: true, message: '请选择广告位置', trigger: 'change' }],
  ...
  methods: {
    getList() {
      ...
      getAdvertList(this.listQuery).then(response => {
    ...
    getFoodSelectorByBrandId(brandId) {
      ...
      this.foodsOptions = []
		...
		handleUpdate(row) {
		  this.advert = Object.assign({}, row) // copy obj
      if(this.advert.url && this.advert.url.indexOf('buyFood') >= 0 ){
        let url = this.advert.url
        this.advert.url = url.split('/')[0]
        this.advert.foodId = parseInt(url.split('/')[1])
      }
      if(this.advert.url && this.advert.url.indexOf('shop') >= 0 ){
        let url = this.advert.url
        this.advert.url = url.split('/')[0]
        this.advert.shopId = parseInt(url.split('/')[1])
      }
		...
		confirm() {
			this.$refs['dataForm'].validate((valid) => {
			  if (valid) {
			    let formData =Object.assign({}, this.advert)
          if(this.advert.url && this.advert.url.indexOf('buyFood') >= 0){
            formData.url = this.advert.url+'/'+this.advert.foodId
          }
          if(this.advert.url && this.advert.url.indexOf('shop') >= 0){
            formData.url = this.advert.url+'/'+this.advert.shopId
          }
					formData.addTime = null
					formData.updTime = null
			    updateAdvert(formData).then(() => {
						this.getList()
		...
		statusChange(row) {
		  let formData = {
			...
			updateAdvertStatus(formData).then(() => {
			  this.$notify({
			    title: '成功',
			    message: '广告开关成功',
			    type: 'success',
